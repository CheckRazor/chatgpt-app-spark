-- Fix run_weighted_distribution_v2: always UPDATE with WHERE id = ...
CREATE OR REPLACE FUNCTION public.run_weighted_distribution_v2(event_uuid uuid, medal_uuid uuid, actor uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_updated integer;
BEGIN
  -- Lock and read event totals with id
  SELECT
    et.id,
    et.total_amount::numeric(30,0),
    COALESCE(et.raffle_amount_used, 0)::numeric(30,0),
    COALESCE(et.distributed_amount, 0)::numeric(30,0),
    COALESCE(et.min_score_for_raffle, 0)
  INTO STRICT v_id, total_pot, raffle_used, already_distributed, min_score_threshold
  FROM public.event_totals et
  WHERE et.event_id = event_uuid AND et.medal_id = medal_uuid
  FOR UPDATE;
  ...
  -- Update distributed amount using WHERE id = v_id
  ...
  IF v_updated IS NULL THEN
    RAISE EXCEPTION 'event_totals update failed (no row matched id=%)', v_id;
  END IF;
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
    RAISE EXCEPTION USING
      errcode = 'P0001',
      message = 'No event_totals row for this (event_id, medal_id).';
END;
$function$;
